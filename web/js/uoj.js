...
uojLocaleData = {
	"username": {
		"en": "Nazwa użytkownika",
		"zh-cn": "Nazwa użytkownika"
	},
	...
	"editor::use advanced editor": {
		"en": "Użyj zaawansowanego edytora",
		"zh-cn": "Użyj zaawansowanego edytora"
	},
	"editor::language": {
		"en": "Język",
		"zh-cn": "Język"
	...
	"editor::upload by editor": {
		"en": "Prześlij edytorem",
		"zh-cn": "Prześlij edytorem"
	},
	"editor::upload from local": {
		"en": "Prześlij lokalny plik",
		"zh-cn": "Prześlij lokalny plik	"
	}
};
...
function validateSettingPassword(str) {
	if (str.length < 6) {
		return 'Długość hasła nie powinna być mniejsza niż 6.';
	} else if (! /^[!-~]+$/.test(str)) {
		return 'Hasło powinno zawierać tylko widoczne znaki ASCII.';
	} else if (str != $('#input-confirm_password').val()) {
		return 'Wprowadzone hasła nie są ze sobą zgodne.';
	} else {
		return '';
	}
}
...
function validateEmail(str) {
	if (str.length > 50) {
		...
		return 'Niedozwolony adres email.';
	} else {
		return '';
	}
}
function validateUsername(str) {
	if (str.length == 0) {
		...
		return 'Nazwa użytkownika powinna zawierać tylko wielkie i małe litery alfabetu angielskiego, cyfry i podkreślenia.';
	} else {
		return '';
	}
}
function validateQQ(str) {
	if (str.length < 5) {
		return 'Długość QQ nie powinna być mniejsza niż 5.';
	} else if (str.length > 15) {
		return 'Długość QQ nie powinna być większa niż 15.';
	} else if (/\D/.test(str)) {
		return 'QQ powinno zawierać tylko cyfry od 0 do 9.';
	} else {
		return '';
	}
}
function validateMotto(str) {
	if (str.length > 50) {
		return 'Motto nie powinno zawierać więcej niż 50 znaków.';
	} else {
		return '';
	}
}
...
$.fn.long_table = function(data, cur_page, header_row, get_row_str, config) {
	return this.each(function() {
		...
		if (now_cnt == 0) {
			tbody.append('<tr><td colspan="233">Brak</td></tr>');
		}
	...
	return this.each(function() {
		...
		var show_help_lang = function() {
			if ($(this).val() == 'Java8' || $(this).val() == 'Java11') {
				div_help_language.text('Uwaga! Pakiet nie powinnien być określony w kodzie źródłowym programu Java. Główna funkcja pierwszej zdefiniowanej klasy zostanie uznana jako punkt wejścia do programu.');
			} else {
			...
			return $('<tr id="' + 'comment-' + reply.id + '" />').append(
				$('<td />').append(
					...
					$('<ul class="text-right list-inline bot-buffer-no" />').append(
						...
						$('<li />').append(
							$('<a href="#">Odpowiedź</a>').click(function (e) {
								e.preventDefault();
								toggleFormReply(reply.id, 'Odpwiedź @' + reply.poster + ':');
							})
						)
					)
				)

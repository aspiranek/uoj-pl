function blog_editor_init(name, editor_config) {
	...
	save_btn.tooltip({ container: 'body', title: 'Zapisz (Ctrl-S)' });
	preview_btn.tooltip({ container: 'body', title: 'Podgląd (Ctrl-D)' });
	bold_btn.tooltip({ container: 'body', title: 'Pogrubienie (Ctrl-B)' });
	italic_btn.tooltip({ container: 'body', title: 'Kursywa (Ctrl-I)' });
	...
	function set_saved(val) {
		...
		if (val) {
			...
			before_window_unload_message = 'Twoje zmiany nie zostały zapisane';
		}
	}
	...
	input_is_hidden.on('switchChange.bootstrapSwitch', function(e, state) {
		var ok = true;
		if (!state && !confirm("Czy na pewno chcesz to upublicznić?")) {
			ok = false;
		}
		if (!ok) {

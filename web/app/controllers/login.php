...
function submitLoginPost() {
	...
	$.post('/login', {
		...
		if (msg == 'ok') {
			...
			$('#help-username').html('Ten użytkownik został zawieszony.');
		} else if (msg == 'expired') {
			$('#div-username').addClass('has-error');
			$('#help-username').html('Sesja strony wygasła.');
		} else {
			$('#div-username').addClass('has-error');
			$('#help-username').html('Zła nazwa użytkownika lub hasło.');
			$('#div-password').addClass('has-error');
			$('#help-password').html('Zła nazwa użytkownika lub hasło. <a href="/forgot-password">Zapomniałeś hasła?</a>');
		}
	});
	return true;
}

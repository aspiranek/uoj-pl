<?php
	...
	if ($hackable) {
		...
		$hack_form->addTextFileInput('input', 'Wejście');
		$hack_form->addCheckBox('use_formatter', 'Proszę, pomóż zachować testy uporządkowane. Pamiętaj o znaku nowej lini na końcu testu i spacji na końcu wiersza.', true);
	...
	if (isSuperUser($myUser)) {
		...
		$delete_form->submit_button_config['text'] = 'Usuń';
...
<?php if ($should_show_content): ?>
	...
	<?php if ($hackable): ?>
		<p class="text-center">
			Ten program wydaje się być nie do końca poprawny... Pozwól mu spróbować z innymi danymi! <button id="button-display-hack" type="button" class="btn btn-danger btn-xs">Hakuj!</button>
		</p>

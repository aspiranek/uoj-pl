...
<?php echoUOJPageHeader('Wiadomości prywatne') ?>

<h1 class="page-header">Wiadomości prywatne</h1>
...
<div id="history" style="display:none">
	<div class="card border-primary">
		<div class="card-header bg-primary text-white">
			<button type="button" id="goBack" class="btn btn-info btn-sm" style="position:absolute">Powrót</button>
			<div id="conversation-name" class="text-center"></div>
		</div>
		<div class="card-body">
			<ul class="pagination top-buffer-no justify-content-between">
				<li class="previous"><a class="btn btn-outline-secondary text-primary" href="#" id="pageLeft">&larr; wcześniejsze wiadomości</a></li>
				<li class="text-center" id="pageShow" style="line-height:32px"></li>
				<li class="next"><a class="btn btn-outline-secondary text-primary" href="#" id="pageRight">późniejsze wiadomości &rarr;</a></li>
			</ul>
			<div id="history-list" style="min-height: 200px;">
			</div>
			<ul class="pagination top-buffer-no justify-content-between">
				<li class="previous"><a class="btn btn-outline-secondary text-primary" href="#history" id="pageLeft2">&larr; wcześniejsze wiadomości</a></li>
				<li class="next"><a class="btn btn-outline-secondary text-primary" href="#history" id="pageRight2">późniejsze wiadomości &rarr;</a></li>
			</ul>
			<hr />
			<form id="form-message">
				...
				<div class="text-right">
					<button type="submit" id="message-submit" class="btn btn-info btn-md">Wyślij</button>
				</div>
			</form>
		</div>
	</div>
	...
	$("#conversations").append(
		'<div class="row top-buffer-sm">' +
			...
			'<div class="col-sm-9" style="line-height:34px">' +
				'Czas ostatniego wysłania: ' + send_time +
			'</div>' +
		'</div>'
	);
}
			...
			'<div style=' + (type ? "margin-left:0%;margin-right:20%;" : "margin-left:20%;margin-right:0%;") + '>' +
				'<div class="card border-info mb-4">' +
					...
					'<div>' +
						'<div class="row">' +
							'<div class="col-sm-6">' +
								'Czas wysłania: ' + send_time +
							'</div>' +
							'<div class="col-sm-6 text-right">' +
								 'Czas odczytania: ' + (read_time == null ? '<strong>Nie przeczytane</strong>' : read_time) +
							'</div>' +
						'</div>' +
					'</div>' +
				'</div>' +
...
function submitMessagePost(conversationName) {
		if ($('#input-message').val().length == 0  ||  $('#input-message').val().length >= 65536) {
				$('#help-message').text('Długość wiadomości prywatnej musi mieścić się w przedziale od 1 do 65535.');
...
function refreshHistory(conversation, page) {
		...
		$('#pageShow').text("Strona " + page.toString());
		$.get('/user/msg', {
				getHistory : '',

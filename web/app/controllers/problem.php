<?php
		...
		if (!hasContestPermission($myUser, $contest)) {
			if ($contest['cur_progress'] == CONTEST_NOT_STARTED) {
				...
				if ($myUser == null || !hasRegistered($myUser, $contest)) {
					becomeMsgPage("<h1>Konkurs w toku!</h1><p>Nie jesteś zarejestrowany!</p>");
				} else {
					$is_in_contest = true;
	...
	if ($can_use_zip_upload) {
		...
		$zip_answer_form->extra_validator = function() {
			...
			if ($ban_in_contest) {
				return 'Poczekaj na koniec konkursu, aż zadanie będzie widoczne dla wszytskich by przesłać.';
			}
			return '';
		};
...
EOD
		...
		$custom_test_form->extra_validator = function() {
			...
			if ($custom_test_submission && $custom_test_submission['status'] != 'Judged') {
				return 'Ostateczna ocena jeszcze się nie zakończyła.';
			}
			return '';
		};
...
<div class="row d-flex justify-content-center">
	<span class="badge badge-secondary mr-1">Limit czasu:<?=$time_limit!=null?"$time_limit s":"N/A"?></span>
	<span class="badge badge-secondary mr-1">Limit pamięci:<?=$memory_limit!=null?"$memory_limit MB":"N/A"?></span>
</div>

<?php
	...
	function handlePost() {
		...
		if (!validatePassword($old_password) || !checkPassword($myUser, $old_password)) {
			return "Błąd: Złe hasło";
		}
		if ($_POST['ptag']) {
			$password = $_POST['password'];
			if (!validatePassword($password)) {
				return "Błąd: Nieprawidłowy format hasła";
			}
		...
		if (!validateEmail($email)) {
			return "Błąd: NIeprawidłowy format adresu email";
		}
		...
		$.post('/user/modify-profile', {
			...
			if (msg == 'ok') {
				BootstrapDialog.show({
					title   : 'Zmodyfikowano pomyślnie',
					message : 'Informacje o użytkowniku zostały pomyślnie zmodyfikowane!',
				...
				BootstrapDialog.show({
					title   : 'Nie udało się zmodyfikować',
					message : msg,

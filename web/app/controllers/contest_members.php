<?php
	...
	if ($contest['cur_progress'] == CONTEST_NOT_STARTED) {
		...
		if ($iHasRegistered) {
			...
			$unregister_form->submit_button_config['text'] = 'Anuluj rejestrację';
		...
		if ($has_contest_permission) {
			...
			$pre_rating_form->submit_button_config['text'] = 'Przelicz ranking przed zawodami';
...
<?php if ($contest['cur_progress'] == CONTEST_NOT_STARTED): ?>
	<?php if ($iHasRegistered): ?>
		...
		<div><a style="color:green">Zarejestrowany</a></div>
	<?php else: ?>
		<div>Obecnie nie jesteś zarejestrowany, możesz<a style="color:red" href="/contest/<?= $contest['id'] ?>/register">zarejestrować się</a>.</div>
	<?php endif ?>

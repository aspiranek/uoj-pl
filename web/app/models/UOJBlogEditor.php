...
class UOJBlogEditor {
	...
	public $label_text = array(
		'title' => 'Tytuł',
		'tags' => 'Tagi (wiele tagów odzielone przecinkami)',
		'content' => 'Treść',
		'view blog' => 'Zobacz bloga',
		'blog visibility' => 'Widoczność bloga',
		'private' => 'Prywatny',
		'public' => 'Publiczny'
	);
	...
	function __construct() {
		...
		$this->validator = array(
			'title' => function(&$title) {
				...
				if (strlen($title) > 100) {
					return 'Długość tytułu nie może być większa niż 100';
				}
			...
			'tags' => function(&$tags) {
				...
				foreach ($tags_raw as $tag) {
					...
					if (strlen($tag) > 30) {
						return 'Tag "' . HTML::escape($tag) .'" jest zbyt długi';
					}
					if (in_array($tag, $tags, true)) {
						return 'Tag "' . HTML::escape($tag) .'" powtarza się';
					}
					$tags[] = $tag;
				}
				return '';
	...
	private function receivePostData() {
		...
		if ($this->type == 'blog') {
			...
			try {
				...
				die(json_encode(array('content_md' => 'Nieznany błąd')));
			}
			...
			if ($content_array === false || !is_array($content_array)) {
				die(json_encode(array('content_md' => 'Nieprawidłowy format YAML')));
			}
...
EOD
	...
	public function handleSave() {
		...
		if (isset($_POST['need_preview'])) {
			ob_start();
			if ($this->type == 'blog') {
				echoUOJPageHeader('Podgląd bloga', array('ShowPageHeader' => false, 'REQUIRE_LIB' => array('mathjax' => '', 'shjs' => '')));
				echo '<article>';

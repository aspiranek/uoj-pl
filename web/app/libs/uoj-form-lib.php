...
EOD;
			$this->add($name, $html,
				function($opt) use ($options) {
					return isset($options[$opt]) ? '' : "Nieprawidłowa opcja";
				},
				null
			);
		}
...
EOD;
		...
		public function addBlogEditor(UOJBlogEditor $editor) {
			...
			$this->addVInput("{$name}_title", 'text', 'Tytuł', $editor->cur_data['title'],
			...
			$content_md_html = HTML::div_vtextarea("{$name}_content_md", 'Treść', $editor->cur_data['content_md']);
				...
				function($type, &$vdata) use ($name) {
					if ($type == 'editor') {
						if (!isset($_POST["{$name}_editor"])) {
							return 'Co robisz? Dlaczego nic nie dodałeś?';
						}
					} elseif ($type == 'file') {
					} else {
						return 'Nie został wprowadzony za pomocą edytora ani przesłany plikiem';
					}
				},
				'always_ok'
			);
...
EOD;
			...
			if (isset($this->submit_button_config['smart_confirm'])) {
				$this->submit_button_config['confirm_text'] = 'Czy jesteś pewien ' . $this->submit_button_config['text'] . '?';
			}
		...
		$(this).find("input[type='file']").each(function() {
			for (var i = 0; i < this.files.length; i++) {
				if (this.files[i].size > 10 * 1024 * 1024) {
					...
					$('#help-' + $(this).attr('name')).text('Rozmiar pliku nie może przekraczać 10M');
					ok = false;
				} else {
		...
		$form->addTextArea(
			...
			function($str, &$vdata) use ($validate) {
				...
				foreach (explode("\n", $str) as $line_id => $raw_line) {
					...
					if ($line[0] != '+' && $line[0] != '-') {
						return 'Wiersz ' . ($line_id + 1) . ': błąd formatu';
					}
					$obj = trim(substr($line, 1));
					
					if ($err = $validate($obj)) {
						return 'Wiersz ' . ($line_id + 1) . ': ' . $err;
					}
			...
			foreach ($requirement as $req) {
				...
				if ($req['type'] == 'source code') {
					...
					if ($stat['size'] > $max_size * 1024) {
						...
						becomeMsgPage("Długość kodu źródłowego nie może przekraczać {$max_size}KB.");
					}
				}
...
EOD;
		...
		$form->handle = function() use ($name, $requirement, $zip_file_name_gen, $handle) {
			...
			if (!isset($_FILES[$name])) {
				becomeMsgPage('Co robisz? Dlaczego nic nie dodałeś?');
			} elseif (!is_uploaded_file($_FILES[$name]['tmp_name'])) {
				becomeMsgPage('Błąd przesyłania...');
			}
			
			$up_zip_file = new ZipArchive();
			if ($up_zip_file->open($_FILES[$name]['tmp_name']) !== true) {
				becomeMsgPage('To nie jest poprawne archiwum ZIP.');
			}
			...
			foreach ($requirement as $req) {
				...
				if ($stat === false) {
					...
					if ($stat['size'] > 20 * 1024 * 1024) {
						becomeMsgPage("Rzeczywisty rozmiar pliku {$req['file_name']} jest za duży/");
					}

...
function becomeMsgPage($msg, $title = 'Infromacja') {
	if (UOJContext::isAjax()) {
		die($msg);
	} else {
...
function become404Page() {
	...
	becomeMsgPage('<div class="text-center"><div style="font-size:233px">404</div><p>Strina nie została znaleziona &gt;_&lt;...</p></div>', '404');
}
function become403Page() {
	header($_SERVER['SERVER_PROTOCOL'] . " 403 Forbidden", true, 403); 
	becomeMsgPage('<div class="text-center"><div style="font-size:233px">403</div><p>Nie masz uprawnień T_T!</p></div>', '403');
}
...
function getSubmissionStatusDetails($submission) {
	...
	$fly = '<img src="/images/utility/qpx_n/b37.gif" alt="Niedźwiadki latają jak superman" class="img-rounded" />';
	$think = '<img src="/images/utility/qpx_n/b29.gif" alt="Niedźwiedź myśli" class="img-rounded" />';
	...
	private function _print($node) {
		if ($node->nodeName == 'error') {
			...
			if ($this->styler->show_small_tip) {
				echo '<div class="text-right text-muted">', 'Wskazówka: Kliknij poziomy pasek, aby zobaczyć szczegółowe informacje.', '</div>';
			} elseif ($this->styler->ioi_contest_is_running) {
				echo '<div class="text-right text-muted">', 'Wyświetlanie szczegółowych informacji nie jest dostępne w konkursach IOI', '</div>';
			}
			$this->_print_c($node);
			echo '</div>';

...
<div class="table-responsive">
	<table class="table table-bordered table-hover table-vertical-middle table-text-center">
		<thead>
			<tr>
				<th style="width:10em">Pytający</th>
				<th style="width:7em">Czas pytania</th>
				<th>Pytanie</th>
			</tr>
		</thead>
		<tbody>
					...
					<td style="text-align: left" class="question" data-qid="<?=$question['id']?>">
						...
						<?php if ($can_reply): ?>
							<div class="text-right">
								<button type="button" class="btn btn-primary btn-xs question-reply-button">Edytuj odpowiedź</button>
							</div>
						<?php endif ?>
						<hr class="top-buffer-sm bot-buffer-md" />
						<?php if ($question['answer'] === ''): ?>
							<div class="question-content"><strong class="text-muted">Jeszcze bez odpowiedzi</strong></div>
						<?php else: ?>
							<div class="question-content uoj-readmore"><strong class="text-danger">Odpwiedź:</strong><span class="text-warning"><?= HTML::escape($question['answer']) ?></span></div>
						<?php endif ?>
					</td>
				</tr>
